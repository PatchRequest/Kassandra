...
struct ResolvedImports {
    list: [ResolvedImport; 512], // Defines the limit of mapped functions, in this case a hard-coded 512.
    len: usize,
}

/// `ResolvedImports` is a struct that contains a list of mapped functions and the length of the list.
impl ResolvedImports {
    ...
    fn new() -> Result<*mut Self, Box<dyn std::error::Error>> {
        Ok(std::ptr::from_mut(unsafe {
            ...
            std::ptr::write_bytes(allocation, 0, core::mem::size_of::<ResolvedImports>());
            &mut *allocation.cast::<ResolvedImports>()
        }))
    }

    /// `push` pushes a mapped function to the list.
    fn push(&mut self, entry: ResolvedImport) {
        self.list[self.len] = entry;
        self.len += 1;
    }
}

/// `Drop` frees the memory allocated for the `ResolvedImports` struct.
impl Drop for ResolvedImports {
    fn drop(&mut self) {
        unsafe {
...
/// `ObjectLoader` is a struct that contains a slice of bytes representing a COFF file and a parsed COFF file.
...
static mut IMPORT_TABLE: Option<&mut ResolvedImports> = None;

/// Static variable that contains the index of the .text section.
static mut CODE_SECTION_INDEX: i32 = 0;

/// Static variable that contains the mapped sections.
static mut SECTION_BASES: Vec<usize> = Vec::new();

impl<'a> ObjectLoader<'a> {
    /// Creates a new `ObjectLoader` struct from a slice of bytes representing a COFF file.
    ...
    fn get_import_from_symbol(&self, symbol: Symbol) -> Result<usize, Box<dyn std::error::Error>> {
        ...
        if unsafe { IMPORT_TABLE.is_none() } {
            unsafe {
                IMPORT_TABLE = Some(&mut *ResolvedImports::new()?);
            }
        }
        ...
        if let Some(index) = polished_import_name.find('$') {
            ...
            let wide: Vec<u16> = library_name_dll.encode_utf16().chain(std::iter::once(0)).collect();
        ...
        let mapped_func_entry = ResolvedImport {
    ...
    fn allocate_bof_memory(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for idx in 0..self.coff.header.number_of_sections {
            ...
            if section.name()?.contains("text") {
                unsafe {
                    CODE_SECTION_INDEX = idx as i32;
                }
            }

            // Push the section base to the section mapping
            unsafe { SECTION_BASES.push(section_base as usize) };
    ...
    fn execute_bof(
        ...
        let mapped_function_names =
            if let Some(function_mapping) = unsafe { IMPORT_TABLE.as_mut() } {
                function_mapping
                    .list
                    .iter()
            ...
            for (_i, name, symbol) in symbols.iter() {
                if let Some(name) = name {
                    ...
                    if name.contains(entry_name.as_str()) {
                        let entry_addr = unsafe { SECTION_BASES[(CODE_SECTION_INDEX) as usize] }
    ...
    fn free_bof_memory(&self) {
        ...
        for (idx, _section) in self.coff.sections.iter().enumerate() {
            let section_base = unsafe { SECTION_BASES[idx] };
